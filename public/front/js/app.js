...
const app = {
  ...
  addNavigationButtonListener(link){
    ...
    link.addEventListener('click', function(event){
      ...
      event.preventDefault(); //przerywa wszystkie domyślne akcje związane ze zdarzeniem.
  ...
  activatePage: function (pageId) { //otrzymujemy informację o id podstrony do aktywacji.
    ...
    for(let page of thisApp.pages){ //powoduje że widzimy odpowiednie podstrony.
      page.classList.toggle(classNames.pages.active, page.id == pageId); //przy toggle można 2 warunków użyć.
    }
    /* add class "active" to matching links, remove from non-matching */
    for(let link of thisApp.navLinks){ //nadawanie bialej barwy tła aktywnej zakładce.
