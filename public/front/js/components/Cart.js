...
class Cart{ //koszyk
  ...
  remove(cartProduct){ //remove from basket
    const thisCart = this;
    const index = thisCart.products.indexOf(cartProduct); //indexOf zwraca pierwszy (najmniejszy) indeks elementu w tablicy równego podanej wartości lub -1, gdy nie znaleziono takiego elementu.
  ...
  update(){ //updated basket
    ...
    for (let oneProduct of thisCart.products){
      thisCart.subtotalPrice += oneProduct.price; // thisCart.subtotalPrice = thisCart.subtotalPrice + oneProduct.price;
