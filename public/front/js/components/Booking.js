...
class Booking {
  ...
  parseData(bookings, eventsCurrent, eventsRepeat){//przetwarza dane z API na 'uzyteczne' dla mnie.
    ...
    for(let booking of bookings){
      for(let tableNumber of booking.tables){ //tu pokazuje kilka stolików zarezerwowanych, nie jeden.
        thisBooking.makeBooked(booking.date, booking.hour, booking.duration, tableNumber);//oznaczam stoliki zarezerwowane.
      }
    }
  ...
  initWidgets(){ //nowe instancje klas.
    ...
    thisBooking.dom.wrapper.addEventListener('updated', function(){ //updated wywoływane przy zmianie daty lub godziny.
  ...
  copyTableWithoutElement(elementsArray, elementToRemove){//odznaczam stolik. Zwracam tabelę bez usuniętego elementu.
    const tablesAfterRemovingDuplicate = [];
    for (let arrayElement of elementsArray){
      if(arrayElement !== elementToRemove){
  ...
  selectTable(table){//zaznaczam wybór stolika.
    ...
    if(inArray === true){//jeśli kliknięty element jest już w tablicy.
      thisBooking.selectedTables = thisBooking.copyTableWithoutElement(thisBooking.selectedTables, table); //nowa tabllica = stara tablica - element.
      table.classList.remove(classNames.booking.tableBooked); //tylko wizualizacja na stronie.
    } else {
  ...
  unSelectTable(){ //odznaczam wybór stolika.
    ...
    thisBooking.selectedTables = []; //"czyszczę" tablicę zakladając nową.
  }
