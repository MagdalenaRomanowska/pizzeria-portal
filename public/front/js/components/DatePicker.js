...
class DatePicker extends BaseWidget{ // DP jest rozszerzeniem klasy BaseWidget.
  ...
  initPlugin(){
    ...
    flatpickr(thisWidget.dom.input, { //zainicjować plugin flatpickr. 1szy arg to input, drugi arg to obiekt zawierający opcje pluginu.
      ...
      onChange: function(selectedDates, dateStr) { //w momencie wykrycia zmiany wartości 
        //przez plugin, chcemy ustawiać wartość właściwości thisWidget.value na dateStr z dokumentacji pluginu.
  ...
  isValid(){ //nie może zostać domyślna z BaseWidget.
    return true; //nie mamy dla niej zastosowania w tym widgecie, ponieważ plugin 
    //zajmie się dbaniem o poprawność danych, więc niech po prostu zwraca true.
  }
